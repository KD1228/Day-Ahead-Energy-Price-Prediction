...
# Function to concatenate all .txt files in the solar dataset
def concat_solar_files():
    dataset_name = "solar"
    dataset_clean_folder = os.path.join(cleaned_folder, dataset_name)  # Path to solar data folder
    dataset_merged_folder = os.path.join(merged_folder, dataset_name)  # Path to save merged solar data
    ...
    if dataframes:
        ...
        merged_file_path = os.path.join(dataset_merged_folder, "solar_merged.txt")
        merged_df.to_csv(merged_file_path, sep=";", index=False, encoding="utf-8")
        print(f"🎉 Merged solar dataset saved at: {merged_file_path}\n")
    else:
        print("⚠️ No valid data found to concatenate!")


# Run concatenation only for solar data
print("\n🚀 Processing solar data ...")
concat_solar_files()
print("\n🎉 Solar dataset concatenated successfully!")
