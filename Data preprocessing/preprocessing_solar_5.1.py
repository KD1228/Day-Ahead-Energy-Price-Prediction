...
file_path = r"C:\Users\kanch\Energy price prediction\Energy price prediction\Merged_Data\solar\solar_merged.txt"
...
# Remove the colon (:) from MESS_DATUM and ensure it's properly formatted (10 digits)
df['MESS_DATUM'] = df['MESS_DATUM'].astype(str).str.replace(":", "").str.zfill(10)
...
filtered_df_solar = df[(df['MESS_DATUM'] >= start_date) & (df['MESS_DATUM'] <= end_date)]

# Check the size of the filtered dataset
print(f"Rows after filtering: {filtered_df_solar.shape[0]}")

# Display the first few rows of the filtered dataframe
print(filtered_df_solar.head())

# Define the output directory and create it if it doesn't exist
output_directory = r"C:\Users\kanch\Energy price prediction\Energy price prediction\Filtered_data\solar"
os.makedirs(output_directory, exist_ok=True)

# Define the file path to save the filtered data
filtered_file_path = os.path.join(output_directory, "solar_filtered.csv")

# Save the filtered data to the new file
filtered_df_solar.to_csv(filtered_file_path, index=False)

# Confirm that the file was saved
print(f"Filtered data saved to: {filtered_file_path}")
print(filtered_df_solar.head(30))