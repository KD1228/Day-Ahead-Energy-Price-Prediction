...
file_path = r"C:\Users\kanch\Energy price prediction\Energy price prediction\Merged_Data\temp\temp_merged.txt"
...
filtered_df_temp = df[(df['MESS_DATUM'] >= start_date) & (df['MESS_DATUM'] <= end_date)]

# Check the size of the filtered dataset
print(f"Rows after filtering: {filtered_df_temp.shape[0]}")

# Display the first few rows of the filtered dataframe
print(filtered_df_temp.head())

# Define the output directory and create it if it doesn't exist
output_directory = r"C:\Users\kanch\Energy price prediction\Energy price prediction\Filtered_data\temp"
os.makedirs(output_directory, exist_ok=True)

# Define the file path to save the filtered data
filtered_file_path = os.path.join(output_directory, "temp_filtered.csv")

# Save the filtered data to the new file
filtered_df_temp.to_csv(filtered_file_path, index=False)

# Confirm that the file was saved
print(f"Filtered data saved to: {filtered_file_path}")

print(filtered_df_temp.head(30))